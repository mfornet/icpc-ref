...
struct randomized_kd_tree
{
	...
	node* build(vector<point> &p, int lo, int hi, bool d) // need fix
	{
		if (lo >= hi)
			return NULL;

		int m = (lo + hi) / 2;
		nth_element(p.begin() + lo, p.begin() + m, p.begin() + hi,
				[&](const point &a, const point &b)
					{ return d ? a.real() < b.real() : a.imag() < b.imag(); });
	...
	node* build2(vector<point> &p, int lo, int hi, bool d)
	{
		...
		int m = partition(p.begin() + lo + 1, p.begin() + hi,
				[&](const point &a)
					{ return d ? a.real() < p[lo].real() : a.imag() < p[lo].imag(); }) - p.begin();
